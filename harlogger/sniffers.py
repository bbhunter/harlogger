        ...
        async for line in self._os_trace_service.syslog():
            ...
            try:
                ...
                entry_hash = EntryHash(
                    line.pid, posixpath.basename(line.filename), os.path.basename(line.image_name), entry.url
                )
...
class SnifferProfileBase(SnifferBase, ABC):
    def _handle_entry(
        ...
        entry_hash = EntryHash(pid, posixpath.basename(filename), os.path.basename(image_name), http_transaction.url)
...
class MobileSnifferProfile(MobileSnifferBase, SnifferProfileBase):
    ...
    async def sniff(self) -> None:
        async for entry in self._os_trace_service.syslog():
            ...
            self._handle_entry(
                entry.pid, entry.message, entry.filename, entry.image_name, subsystem=subsystem, category=category
            )
